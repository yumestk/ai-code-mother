...
import com.yume.yuaicodemother.ai.AiCodeGenTypeRoutingService;
import com.yume.yuaicodemother.annotation.AuthCheck;
import com.yume.yuaicodemother.common.BaseResponse;
import com.yume.yuaicodemother.common.DeleteRequest;
...
import com.yume.yuaicodemother.model.dto.app.*;
...
public class AppController {
    ...
    public Flux<ServerSentEvent<String>> chatToGenCode(@RequestParam Long appId,
                                                       @RequestParam String messge,
                                                       HttpServletRequest request) {
        // 参数校验
        ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用 ID 错误");
        ThrowUtils.throwIf(StrUtil.isBlank(messge), ErrorCode.PARAMS_ERROR, "提示词不能为空");
        // 获取当前登录用户
        User loginUser = userService.getLoginUser(request);
        // 调用服务生成代码 (SSE 流式返回)
        Flux<String> contentFlux = appService.chatToGenCode(appId, messge, loginUser);
